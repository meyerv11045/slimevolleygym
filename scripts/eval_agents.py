...
if __name__=="__main__":
  ...
  left, right = evaluate_multiagent(env, policy0, policy1,
    render_mode=render_mode, n_trials=args.trials, init_seed=args.seed)

  print("left history dump: ", left)
  print("right history dump: ", right)
  print(f"over {args.trials} trials:")
  print(f"Left agent scored {np.round(np.mean(left), 3)} ± {np.round(np.std(left), 3)} vs right agent")
  print(f"Right agent scored {np.round(np.mean(right), 3)} ± {np.round(np.std(right), 3)} vs left agent")