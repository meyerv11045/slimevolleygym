...
REF_WALL_HEIGHT = 8
PLAYER_SPEED_X = 15*1.75
PLAYER_SPEED_Y = 20*1.35
MAX_BALL_SPEED = 20*1.5
TIMESTEP = 1/30.
NUDGE = 0.1
FRICTION = 0.5 # 1 means no FRICTION, less means FRICTION
...
class VariantPixelEnv(VariantEnv):
  from_pixels = True

class VariantAtariEnv(VariantEnv):
  from_pixels = True
  atari_mode = True

class VariantSurvivalAtariEnv(VariantEnv):
  from_pixels = True
  atari_mode = True
  survival_bonus = True

class VariantEnvPPOExpert(VariantEnv):
  def __init__(self):
    super().__init__()
    # expert policy to play against
    self.policy = PPOPolicy()

class VariantEnvGAExpert(VariantEnv):
  def __init__(self):
    super().__init__()
    # expert policy to play against
    self.policy = makeSlimePolicyLite()

class VariantEnvCMAExpert(VariantEnv):
    def __init__(self):
      super().__init__()
      # expert policy to play against
